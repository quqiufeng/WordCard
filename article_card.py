...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{
            font-family: 'SourceHanSans', 'SimSun', 'Microsoft YaHei', sans-serif;
            font-size: 14px;
            line-height: 1.8;
            color: {COLORS['text']};
...
<body>
    ...
    <div class="page">
        ...
        <div class="content">{article['original'].replace(chr(10), '<br>')[:2000]}</div>
        <div class="footer">Page 2</div>
    </div>
    <div class="page">
        <div class="section-title">译文</div>
        <div class="content">{article['translation'].replace(chr(10), '<br>')[:2000]}</div>
...
"""
        ...
        for v in article['vocabulary'][:20]:
            pdf_html += f"""
        <div class="sentence">
            <div class="sentence-en"><b>{v['word']}</b> <span style="color:{COLORS['accent']}">{v['pos']}</span></div>
...
"""
        ...
        for i, s in enumerate(article['sentences'][:5], 1):
            pdf_html += f"""
        <div class="sentence">
            <div class="sentence-en">{i}. {s['original']}</div>
