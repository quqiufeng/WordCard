...
def create_cards(parsed_file, base_name):
    """调用 article_card.py 生成卡片"""
    os.system(f"python3 article_card.py {parsed_file}")

def main():
    if len(sys.argv) < 2:
        print("用法: python generate_cards.py article.txt [标题]")
    ...
    print("\n生成卡片...")
    create_cards(parsed_file, base_name)

    print("\n完成！")
    print(f"输出目录: output/")
    print(f"  {base_name}_one_shot.txt  - 原始输出")
    print(f"  {base_name}_parsed.txt   - 解析后")
    print(f"  {base_name}_parsed.md    - Markdown")
    print(f"  {base_name}_parsed.png   - 图片")
    print(f"  {base_name}_parsed.pdf   - PDF")
